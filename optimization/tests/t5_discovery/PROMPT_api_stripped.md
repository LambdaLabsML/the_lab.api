...
**Start every session by orienting** — `GET /orient` returns active ideas, running experiments, best score, and recommended next steps.
...
2. **Leaderboard** → `GET /leaderboard?metric=score` — rankings of ideas by metric.
...
- **Don't GET individual ideas** — `/leaderboard` already includes the best idea's details. Reading ideas one-by-one wastes your budget.
- **Don't GET individual experiments** — `/wait` returns the full result.
- Use `/orient` → `/leaderboard` → act. Two calls give you everything you need to decide.
